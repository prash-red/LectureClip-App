...
parser.add_argument("--math-epochs", type=int, default=2, help="number of epochs of MATH (competition math) in training mixture")
parser.add_argument("--aqua-epochs", type=int, default=1, help="number of epochs of AQuA-RAT in training mixture")
parser.add_argument("--metamath-epochs", type=int, default=1, help="number of epochs of MetaMathQA in training mixture")
...
# Paths to additional math datasets (prepared by scripts/prepare_math_data.py)
math_train_path = os.path.join(base_dir, "math_train.jsonl")
aqua_train_path = os.path.join(base_dir, "aqua_train.jsonl")
metamath_train_path = os.path.join(base_dir, "metamath_train.jsonl")
...
train_tasks = [
    ...
    *[CustomJSON(filepath=math_train_path) for _ in range(args.math_epochs)],     # ~12K rows per epoch (competition math)
    *[CustomJSON(filepath=aqua_train_path) for _ in range(args.aqua_epochs)],     # ~97K rows per epoch (algebra word problems)
    *[CustomJSON(filepath=metamath_train_path) for _ in range(args.metamath_epochs)], # 50K rows per epoch (augmented GSM8K/MATH)
]
train_dataset = TaskMixture(train_tasks)
print0(f"Training mixture: {len(train_dataset):,} rows (MMLU x{args.mmlu_epochs}, GSM8K x{args.gsm8k_epochs}, MATH x{args.math_epochs}, AQuA x{args.aqua_epochs}, MetaMath x{args.metamath_epochs})")
...
while True:
    ...
    if last_step:
        output_dirname = (args.model_tag + "_math") if args.model_tag else f"d{depth}_math" # e.g. d24_math
