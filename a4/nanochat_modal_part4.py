...
Individual stages (if you want to re-run one step):
    ...
    modal run nanochat_modal.py::stage_post_rl_eval
...
def stage_rl(wandb_run: str = WANDB_RUN) -> None:
    ...
    _torchrun("scripts.chat_eval", ["-i", "rl", "-a", "GSM8K"], nproc=_N_FINETUNE_GPUS)
...
# =============================================================================
# STAGE: POST-RL EVALUATION (detailed logging)
# =============================================================================
...
def stage_post_rl_eval() -> None:
    """
    Run chat_eval on the RL checkpoint with a single GPU so that all GSM8K
    problems are evaluated by one rank and saved to a single JSONL file.

    This produces eval_details_GSM8K_rl.jsonl (and any other generative task
    details) inside the nanochat cache directory for downstream analysis.

    Requires the modified chat_eval.py that logs per-problem details.
    ...
    # Run chat_eval on the RL checkpoint, single GPU, GSM8K only
    print("Evaluating RL checkpoint on GSM8K (single GPU, full detail logging)...")
    _torchrun(
        "scripts.chat_eval",
        [
            "-i", "rl",
    ...
    # Copy the generated JSONL to a convenient location in the volume
    import shutil
    detail_src = os.path.join(NANOCHAT_CACHE, "eval_details_GSM8K_rl.jsonl")
    detail_dst = os.path.join(NANOCHAT_CACHE, "analysis", "eval_details_GSM8K_rl.jsonl")
    if os.path.exists(detail_src):
        os.makedirs(os.path.dirname(detail_dst), exist_ok=True)
        shutil.copy2(detail_src, detail_dst)
        print(f"Copied detail log to {detail_dst}")
    else:
        print(f"Warning: expected {detail_src} not found — check chat_eval modifications")

    volume.commit()
    print("Post-RL evaluation complete. Detail logs saved to volume.")



# =============================================================================
# STAGE: RL Run 1 — Step + Proximity combined
# =============================================================================
...
def stage_rl_step_proximity(wandb_run: str = "8h100_d24_rl_step_proximity") -> None:
    """
    RL with both additional rewards (step bonus + proximity).
    
    Loads from the same SFT checkpoint as Run 0, saves checkpoints to
    chatrl_checkpoints/d24_rl_step_proximity/ to avoid overwriting the original.
    """
    _setup_cache()
 
    print("Running RL with step + proximity rewards...")
    _torchrun(
        "scripts.chat_rl_custom",
        [
            f"--run={wandb_run}",
            "--reward-mode=step_proximity",
            "--model-step=483",
            "--model-tag=d24",
            "--output-tag=d24_rl_step_proximity",
        ],
        nproc=_N_FINETUNE_GPUS,
    )
 
    volume.commit()
    print("RL Run 1 (step_proximity) complete.")
 
 
# =============================================================================
# STAGE: RL Run 2 — Step only
# =============================================================================
...
def stage_rl_step(wandb_run: str = "8h100_d24_rl_step") -> None:
    """
    RL with step reward only (reasoning process bonus).
    
    Same SFT starting point, separate checkpoint directory.
    """
    _setup_cache()
 
    print("Running RL with step reward only...")
    _torchrun(
        "scripts.chat_rl_custom",
        [
            f"--run={wandb_run}",
            ...
            "--model-step=483",
            "--model-tag=d24",
            "--output-tag=d24_rl_step",
        ],
        nproc=_N_FINETUNE_GPUS,
    )
 
    volume.commit()
    print("RL Run 2 (step) complete.")
 
 
# =============================================================================
# STAGE: RL Run 3 — Proximity only
# =============================================================================
...
def stage_rl_proximity(wandb_run: str = "8h100_d24_rl_proximity") -> None:
    """
    RL with proximity reward only (numerical closeness partial credit).
    
    Same SFT starting point, separate checkpoint directory.
    """
    _setup_cache()
 
    print("Running RL with proximity reward only...")
    _torchrun(
        "scripts.chat_rl_custom",
        [
            f"--run={wandb_run}",
            "--reward-mode=proximity",
            "--model-step=483",
            "--model-tag=d24",
            "--output-tag=d24_rl_proximity",
        ],
        nproc=_N_FINETUNE_GPUS,
    )
 
    volume.commit()
    print("RL Run 3 (proximity) complete.")
 
 
# =============================================================================
# STAGE: Post-RL Eval for Run 1 (step + proximity)
# =============================================================================
...
def stage_post_rl_eval_step_proximity() -> None:
    """
    Evaluate Run 1 (step_proximity) checkpoint.
    Saves to eval_details_GSM8K_rl_step_proximity.jsonl and
    report section 'Chat evaluation rl_step_proximity'.
    """
    _setup_cache()
 
    print("Evaluating RL checkpoint (step + proximity)...")
    _torchrun(
        "scripts.chat_eval_detail",
        [
            "-i", "rl",
            "--model-tag", "d24_rl_step_proximity",
            "-a", "GSM8K",
            "--run-label", "rl_step_proximity",
        ],
        nproc=_N_FINETUNE_GPUS,
    )
    ...
    import shutil
    base = NANOCHAT_CACHE
    src = os.path.join(base, "eval_details_GSM8K_rl_step_proximity.jsonl")
    dst = os.path.join(base, "analysis", "eval_details_GSM8K_rl_step_proximity.jsonl")
    ...
    volume.commit()
    print("Post-RL eval (step_proximity) complete.")
 
 
# =============================================================================
# STAGE: Post-RL Eval for Run 2 (step only)
# =============================================================================
...
def stage_post_rl_eval_step() -> None:
    """
    Evaluate Run 2 (step only) checkpoint.
    Saves to eval_details_GSM8K_rl_step.jsonl and
    report section 'Chat evaluation rl_step'.
    """
    _setup_cache()
 
    print("Evaluating RL checkpoint (step)...")
    _torchrun(
        "scripts.chat_eval_detail",
        [
            "-i", "rl",
            "--model-tag", "d24_rl_step",
            "-a", "GSM8K",
            "--run-label", "rl_step",
        ],
        nproc=_N_FINETUNE_GPUS,
    ...
    import shutil
    base = NANOCHAT_CACHE
    src = os.path.join(base, "eval_details_GSM8K_rl_step.jsonl")
    dst = os.path.join(base, "analysis", "eval_details_GSM8K_rl_step.jsonl")
    if os.path.exists(src):
        os.makedirs(os.path.dirname(dst), exist_ok=True)
        shutil.copy2(src, dst)
 
    volume.commit()
    print("Post-RL eval (step) complete.")
 
 
# =============================================================================
# STAGE: Post-RL Eval for Run 3 (proximity only)
# =============================================================================
...
def stage_post_rl_eval_proximity() -> None:
    """
    Evaluate Run 3 (proximity only) checkpoint.
    Saves to eval_details_GSM8K_rl_proximity.jsonl and
    report section 'Chat evaluation rl_proximity'.
    """
    _setup_cache()
 
    print("Evaluating RL checkpoint (proximity)...")
    _torchrun(
        "scripts.chat_eval_detail",
        [
            "-i", "rl",
            "--model-tag", "d24_rl_proximity",
            "-a", "GSM8K",
            "--run-label", "rl_proximity",
        ],
        nproc=_N_FINETUNE_GPUS,
    )
    ...
    import shutil
    base = NANOCHAT_CACHE
    src = os.path.join(base, "eval_details_GSM8K_rl_proximity.jsonl")
    dst = os.path.join(base, "analysis", "eval_details_GSM8K_rl_proximity.jsonl")
    if os.path.exists(src):
        os.makedirs(os.path.dirname(dst), exist_ok=True)
        shutil.copy2(src, dst)
 
    volume.commit()
    print("Post-RL eval (proximity) complete.")
...
def main() -> None:
    ...
    # print("[5/6] RL fine-tuning...")
    # stage_rl.remote(wandb_run=WANDB_RUN)
