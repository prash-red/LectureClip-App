...
# =============================================================================
# STAGE: SUPERVISED FINE-TUNING WITH MATH
# =============================================================================
@app.function(
    image=image,
    secrets=[secret],
...
def stage_sft_math(wandb_run: str = "8h100_d24_math_sft") -> None:
    """
    SFT with additional math datasets (MATH, AQuA-RAT, MetaMathQA).
    
    This stage:
    1. Downloads and prepares the math datasets into JSONL format
    2. Runs chat_sft.py with the additional datasets mixed in
    3. Saves checkpoints to a SEPARATE directory using --model-tag=d24_math
    
    Checkpoints are saved to:
        nanochat_cache/chatsft_checkpoints/d24_math/model_XXXXXX.pt
    (vs the original SFT which saves to d24/)
    """
    _setup_cache()
 
    # --- Step 1: Download identity conversations (same as original) ---
    identity_dest = os.path.join(NANOCHAT_CACHE, "identity_conversations.jsonl")
    print("Downloading identity conversations...")
    _curl(IDENTITY_JSONL_URL, identity_dest)
 
    # --- Step 2: Prepare math datasets ---
    # Check if datasets are already prepared (idempotent)
    math_train = os.path.join(NANOCHAT_CACHE, "math_train.jsonl")
    aqua_train = os.path.join(NANOCHAT_CACHE, "aqua_train.jsonl")
    metamath_train = os.path.join(NANOCHAT_CACHE, "metamath_train.jsonl")
    
    if not os.path.exists(math_train) or not os.path.exists(aqua_train) or not os.path.exists(metamath_train):
        print("Preparing additional math datasets...")
        # Install datasets library if not already available
        # _run("pip install datasets --break-system-packages 2>/dev/null || uv pip install datasets")
        _python("scripts.prepare_math_data", [
            f"--output-dir={NANOCHAT_CACHE}",
        ])
        volume.commit()
    else:
        print("Math datasets already prepared, skipping download.")
 
    # --- Step 3: Run modified SFT with math datasets ---
    # Uses --model-tag=d24_math so checkpoints save to a SEPARATE directory
    # from the original SFT (which used --model-tag=d24)
    print("Running math-augmented SFT...")
    _torchrun(
        "scripts.chat_sft_math",  # Use the modified script
        [
            f"--run={wandb_run}",
            "--model-step=7308",        # Same pretrained checkpoint
            "--model-tag=d24",          # Load FROM the d24 base checkpoint
            # Note: handle the output tag in the script itself
        ],
        nproc=_N_FINETUNE_GPUS,
    )
 
    # --- Step 4: Evaluate the math SFT checkpoint ---
    print("Evaluating math-SFT checkpoint...")
    _torchrun("scripts.chat_eval",
              ["-i", "sft", "--model-tag", "d24_math"],
              nproc=_N_FINETUNE_GPUS)
 
    volume.commit()
    print("Math-augmented SFT complete.")
...
def stage_rl(wandb_run: str = WANDB_RUN) -> None:
    ...
    # _torchrun("scripts.chat_eval", ["-i", "rl", "-a", "GSM8K"], nproc=_N_FINETUNE_GPUS) # commented out since we have a separate post-RL eval stage with detailed logging
...
def stage_post_rl_eval() -> None:
    ...
    _torchrun(
        "scripts.chat_eval_detail", # custom version of chat_eval that logs detailed per-problem info to JSONL
        [
            "-i", "rl",
            "-a", "GSM8K",
        ],
